...
class StudentControllerTest {
  ...
  void getCoursesWithStatus_returnsCorrectJson() throws Exception {
    ...
    List<StudentCourseStatusDetail> expectedList = Collections.emptyList();
  ...
  // ★★★ 修正箇所（バリデーション検証テストの追加） ★★★
  @Test
  @DisplayName("申込状況検索APIで空文字を渡すと400 Bad Requestが返されること")
  void getStudentsByStatus_shouldReturnBadRequest_whenStatusIsBlank() throws Exception {
    // 1. テスト対象のURLに空白文字を渡す
    String blankStatus = " ";

    mockMvc.perform(get("/students/status")
            .param("applicationStatus", blankStatus) // 空白のパラメータを渡す
            .contentType(MediaType.APPLICATION_JSON))

        // 2. 期待される結果: HTTPステータス 400 Bad Request
        .andExpect(status().isBadRequest())

        // 3. 期待されるメッセージの一部が含まれていること（ConstraintViolationExceptionのハンドリングを確認）
        .andExpect(content().string(containsString("申込状況は空にできません。")));

    // サービス層のメソッドが呼び出されないこと（Controllerのバリデーションで弾かれているため）
    verify(service, never()).findStudentDetailsByApplicationStatus(anyString());
  }
  // ★★★ 修正箇所（追加箇所ここまで） ★★★
